...
import profileHook from "../../Hooks/employeeHook/profileHook.js";

const EmployeeProfile = () => {

  const {createProfile, isProfileCreated, EditProfile, employeeId, HandleInputValidation, HandleSaveprofileSubmit, HandleEditMode, HandleUpdateProfile} = profileHook();
        ...
        <div className="Profile-form">
          ...
          <section className="apply-form-contain p-5 flex flex-col gap-2">
            ...
            <div className="leave-button bg-gradient-to-r from-pink-800 to-indigo-900 rounded-full w-full md:p-6 p-4 text-center mt-5">
              {!isProfileCreated? (<button onClick={HandleSaveprofileSubmit}>Save Profile</button>):!EditProfile?(<button onClick={HandleEditMode}>Edit Your Profile</button>):(<button onClick={()=>{HandleUpdateProfile(employeeId)}}>Update Your Profile</button>)}
            </div>
          </section>
        </div>
      </div>
